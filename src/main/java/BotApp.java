            ...
            Mono<Void> onMessage = gateway.on(MessageCreateEvent.class, event -> {
                ...
                if (message.getContent().charAt(0) == config.getCommandCharacter()
                    ...
                    switch (parseCommand(message.getContent())) {
                        case "gpt":
                            String response;
                            if (config.getOpenAiModel().equals(OpenAiHelper.OpenAiModels.GPT_3_5_TURBO.getId())) {
                                response = aiHelper.makeOpenAiChatCompletionRequest(query);
                            } else {
                                response = aiHelper.makeOpenAiCompletionRequest(query);
                            }
